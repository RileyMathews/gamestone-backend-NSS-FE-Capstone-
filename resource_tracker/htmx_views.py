...
def roll_dice_hx(
    ...
    roll_log = models.RollLog.objects.get_or_create(game_player=game_player)[0]
...
def archive_rolls_hx(request: HttpRequest, game_instance_id: str):
    game_player = models.GamePlayer.objects.get(
        game_instance=game_instance_id, player=request.user
    )
    roll_log = get_object_or_404(models.RollLog, game_player=game_player)
...
def resource_instance_edit_hx(request: HttpRequest, id: str):
    ...
    return TemplateResponse(
        ...
        "resource_tracker/hx/resource_instance_edit.html",
        {
            "form": form,
            "resource": resource,
            "input_field_id": f"id_current_ammount-{resource.id}",
        },
    )
