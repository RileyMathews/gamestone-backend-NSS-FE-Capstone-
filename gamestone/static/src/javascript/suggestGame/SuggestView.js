...
class SuggestView extends Component {
    ...
    suggestGameBySimilarity = function (filters) {
        ...
        const gameToSuggest = ArrayManager.getRandomUnownedGame(selectedGame.similar_games, this.state.userGames.map(g => g.id))
    ...
    suggestGameByDeveloper = function (filters) {
        ...
        APIManager.getGbCompany(selectedDeveloper.id)
            .then(response => {
                ...
                const selectedGame = ArrayManager.getRandomUnownedGame(developerGames, this.state.userGames.map(g => g.id))
